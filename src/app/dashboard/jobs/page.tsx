...
function CustomDatePicker({ value, onChange, label }: { value: string, onChange: (v: string) => void, label: string }) {
  ...
  return (
    <div className="relative">
      ...
      >
        <span className={`font-medium text-sm transition-colors ${value ? 'text-foreground' : 'text-foreground/50'}`}>
          ...
          <>
            ...
            >
              <div className="flex items-center justify-between mb-6">
                ...
                <span className="font-medium text-sm tracking-wide">{currentMonthName}</span>
              ...
              <div className="grid grid-cols-7 gap-1 text-center mb-3">
                {['Su', 'Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa'].map(d => <span key={d} className="text-[10px] font-medium text-foreground/30 uppercase">{d}</span>)}
              </div>
              ...
              {times.map(t => {
                ...
                return (
                  <button 
                    ...
                    className={`w-full text-left px-4 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 mb-1 border ${
                      isSelected 
...
export default function JobsManagement() {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-4 relative">
      <PageHeader
        eyebrow={isClientUser ? "Client jobs" : "Operations board"}
        title="Jobs"
        description={isClientUser ? "Create jobs for your venues and track admin-reviewed applicants." : "Create jobs, review applicants, confirm assignments, and keep shifts manual-first."}
        actions={
          <button
            type="button"
      ...
      <WorkspaceCard padding="none" className="flex flex-col overflow-hidden">
        <div className="p-3 border-b border-border flex flex-col md:flex-row gap-3 justify-between items-center bg-card">
          <SearchInput value={searchQuery} onChange={setSearchQuery} placeholder="Search by role, venue, or ID..." className="md:max-w-sm" />
          <FilterBar
            filters={["All", "Open", "PendingReview", "Filled", "Cancelled"]}
            active={filter}
            onChange={(value) => setFilter(value as JobStatus | "All" | "PendingReview")}
          />
        </div>
                ...
                >
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      <div className={`p-1.5 rounded-lg ${job.status === "Filled" ? "bg-emerald-500/10 text-emerald-500" : "bg-primary/10 text-primary"}`}>
                        <Briefcase className="w-4 h-4" />
                      </div>
                      <div>
                        <div className="flex items-center gap-2">
                          <p className="font-medium group-hover:text-primary transition-colors">{job.role}</p>
                          {job.isUrgent && <span className="px-2 py-0.5 bg-amber-500/10 text-amber-500 text-[10px] font-medium uppercase rounded border border-amber-500/20">Urgent</span>}
                        </div>
                  ...
                  <td className="px-4 py-3">
                    <p className="font-medium text-emerald-500">${job.hourlyRate.toFixed(2)}<span className="text-xs text-foreground/50 font-normal">/hr</span></p>
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
          ...
          <>
            ...
            >
              ...
              <form onSubmit={handleCreateJob} className="p-6 overflow-y-auto space-y-6">
                ...
                {createSuccess && (
                  <div className="p-3 rounded-xl border border-emerald-500/30 bg-emerald-500/10 text-emerald-600 text-sm font-medium">
                    {createSuccess}
                  </div>
                )}
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    <label className="block text-xs font-medium text-foreground/70 mb-1">ASSOCIATED CLIENT</label>
                  ...
                  <div>
                    <label className="block text-xs font-medium text-foreground/70 mb-1">VENUE / LOCATION</label>
                  ...
                  <div>
                    <label className="block text-xs font-medium text-foreground/70 mb-1">ROLE / TITLE</label>
                ...
                {showAddVenueInline && (
                  <div className="p-4 rounded-2xl border border-primary/20 bg-primary/5 space-y-3">
                    <p className="text-sm font-medium text-primary">Create New Venue</p>
                    ...
                    />
                    {venueInlineError && (
                      <div className="text-xs font-medium text-red-600">{venueInlineError}</div>
                    )}
                    <div className="flex justify-end">
                      <button
                        ...
                        className="px-4 py-2 bg-primary text-primary-foreground rounded-lg font-medium text-sm disabled:opacity-60"
                      >
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    <label className="block text-xs font-medium text-foreground/70 mb-1">HOURLY RATE ($)</label>
                    <div className="relative flex items-center bg-secondary/10 border border-secondary rounded-xl overflow-hidden focus-within:border-primary focus-within:ring-1 focus-within:ring-primary transition-all group">
                      ...
                      <input 
                        ...
                        className="w-full bg-transparent px-3 py-2.5 focus:outline-none font-medium text-sm" 
                      />
                    </div>
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-foreground/70 mb-1">WORKERS NEEDED</label>
                    <input
                      required
                      type="number"
                      min={1}
                      ...
                      className="w-full px-4 py-2.5 bg-secondary/10 border border-secondary rounded-xl focus:outline-none focus:border-primary font-medium text-sm"
                    />
                  ...
                  <div className="flex items-center md:mt-6">
                    <label className="flex items-center justify-center gap-3 cursor-pointer p-2.5 border border-amber-500/30 bg-amber-500/5 rounded-xl w-full hover:bg-amber-500/10 transition-colors">
                      ...
                      <span className="font-medium text-amber-500 text-sm uppercase tracking-wider">Mark as Urgent Job</span>
                    </label>
                  </div>
                </div>

                <div className="space-y-4 pt-4 border-t border-secondary">
                  <h3 className="font-medium text-lg">Detailed Information</h3>
                  
                  <div>
                    <label className="block text-xs font-medium text-foreground/70 mb-1">ARRIVAL INSTRUCTIONS & DUTIES</label>
                  ...
                  <div>
                    <label className="block text-xs font-medium text-foreground/70 mb-1">UNIFORM / DRESS CODE</label>
                  ...
                  <div>
                    <label className="block text-xs font-medium text-foreground/70 mb-1">PARKING INFORMATION</label>
                ...
                <div className="pt-6 border-t border-secondary flex justify-end gap-4">
                  <button type="button" onClick={() => setIsCreating(false)} className="px-6 py-2.5 border border-secondary rounded-xl font-medium hover:bg-secondary/50 transition-colors">
                    Cancel
                  </button>
                  <button type="submit" className="px-8 py-2.5 bg-primary text-primary-foreground font-medium rounded-xl shadow-lg shadow-primary/20 hover:bg-primary/90 transition-all">
            ...
            >
              <div className="p-6 border-b border-secondary flex justify-between items-start bg-secondary/5">
                <div className="w-full mr-4">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-xs font-medium text-foreground/50 font-mono tracking-wider">{selectedJob.id}</span>
                    ...
                    {selectedJob.isUrgent && <span className="px-2 py-0.5 bg-amber-500/10 text-amber-500 text-[10px] font-medium uppercase rounded border border-amber-500/20">Urgent</span>}
                  </div>
                  {isEditing ? (
                    <input 
                      type="text" 
                      ...
                      className="text-2xl font-medium bg-secondary/20 border border-primary rounded-lg px-3 py-1 w-full mb-2 focus:outline-none focus:ring-1 focus:ring-primary transition-all" 
                    />
                  ) : (
                    <h2 className="text-2xl font-medium">{selectedJob.role}</h2>
                  )}
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  {selectedJob.status === "Open" && (
                    <button 
                      ...
                      className={`px-4 py-2 rounded-xl text-sm font-medium flex items-center gap-2 transition-all ${isEditing ? "bg-emerald-500 text-white shadow-lg shadow-emerald-500/20" : "border border-secondary hover:bg-secondary/50"}`}
                    >
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-8">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className={`p-5 border rounded-2xl bg-background glass flex flex-col justify-center transition-all duration-300 ${isEditing ? 'border-primary shadow-lg shadow-primary/5' : 'border-secondary'}`}>
                    <div className={`flex items-center gap-2 text-xs font-medium mb-3 ${isEditing ? 'text-primary' : 'text-foreground/50'}`}>
                    ...
                    {isEditing ? (
                      ...
                      <>
                        <p className="font-medium text-lg mb-1">{formatDate(selectedJob.date)}</p>
                        <p className="text-sm font-medium text-foreground/70 flex items-center gap-1.5">
                          ...
                          <span className="ml-1 px-1.5 py-0.5 bg-secondary rounded text-xs font-medium">{selectedJob.hours}h</span>
                        </p>
                      </>
                    )}
                  </div>
                  ...
                  <div className={`p-5 border rounded-2xl bg-background glass flex flex-col justify-between transition-all duration-300 ${isEditing ? 'border-primary shadow-lg shadow-primary/5' : 'border-secondary'}`}>
                    <div className={`flex items-center gap-2 text-xs font-medium mb-2 ${isEditing ? 'text-primary' : 'text-foreground/50'}`}>
                    ...
                    {isEditing ? (
                      <div className="mt-2">
                        <div className="flex items-center bg-secondary/10 border border-secondary rounded-lg px-3 py-2 focus-within:border-primary transition-all">
                          <span className="text-emerald-500 font-medium text-xl mr-1">$</span>
                          <input type="number" min="15" step="0.5" value={editForm.hourlyRate || ""} onChange={e => setEditForm({...editForm, hourlyRate: Number(e.target.value)})} className="w-full bg-transparent focus:outline-none font-medium text-emerald-500 text-xl" />
                        </div>
                        <label className="flex items-center justify-center gap-2 mt-4 cursor-pointer p-2 border border-amber-500/30 bg-amber-500/5 rounded-lg hover:bg-amber-500/10 transition-colors">
                          ...
                          <span className="font-medium text-amber-500 text-xs">Mark Urgent</span>
                        </label>
                      </div>
                    ) : (
                      <div className="mt-auto">
                        <p className="font-medium text-emerald-500 text-4xl">${selectedJob.hourlyRate.toFixed(2)}</p>
                        <p className="text-sm font-medium text-foreground/50 uppercase tracking-widest mt-1">Per Hour</p>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div>
                  <h4 className="text-sm font-medium text-foreground/50 mb-4 uppercase tracking-wider">Detailed Information</h4>
                  <div className="space-y-3">
                    ...
                    <div className={`p-4 rounded-xl border flex gap-4 transition-all ${isEditing ? 'bg-background border-primary shadow-sm shadow-primary/10' : 'bg-secondary/10 border-secondary'}`}>
                      ...
                      <div className="flex-1">
                        <p className={`text-xs font-medium mb-1 ${isEditing ? 'text-primary' : 'text-foreground/50'}`}>ARRIVAL INSTRUCTIONS</p>
                        {isEditing ? (
                    ...
                    <div className={`p-4 rounded-xl border flex gap-4 transition-all ${isEditing ? 'bg-background border-primary shadow-sm shadow-primary/10' : 'bg-secondary/10 border-secondary'}`}>
                      ...
                      <div className="flex-1">
                        <p className={`text-xs font-medium mb-1 ${isEditing ? 'text-primary' : 'text-foreground/50'}`}>UNIFORM / DRESS CODE</p>
                        {isEditing ? (
                    ...
                    <div className={`p-4 rounded-xl border flex gap-4 transition-all ${isEditing ? 'bg-background border-primary shadow-sm shadow-primary/10' : 'bg-secondary/10 border-secondary'}`}>
                      ...
                      <div className="flex-1">
                        <p className={`text-xs font-medium mb-1 ${isEditing ? 'text-primary' : 'text-foreground/50'}`}>PARKING INFO</p>
                        {isEditing ? (
                ...
                {!isEditing && (
                  <div className="border-t border-secondary pt-8">
                    ...
                    {selectedJob.status === "Filled" ? (
                      <div>
                        <h4 className="text-sm font-medium text-emerald-500 mb-4 flex items-center gap-2">
                        ...
                        <div className="p-4 border border-emerald-500/30 bg-emerald-500/5 rounded-xl flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-full bg-emerald-500/20 flex items-center justify-center font-medium text-emerald-500">
                              {selectedJob.assignedWorkerName?.charAt(0)}
                            </div>
                            <div>
                              <p className="font-medium">{selectedJob.assignedWorkerName}</p>
                          ...
                          {canManageAssignments && (
                            <div className="flex gap-2">
                              <button 
                                ...
                                className="px-4 py-2 border border-red-500/30 text-red-500 bg-red-500/10 text-xs font-medium rounded-lg hover:bg-red-500/20 transition-colors disabled:opacity-50"
                              >
                                Unassign
                              </button>
                              <button type="button" className="px-4 py-2 border border-secondary text-xs font-medium rounded-lg hover:bg-secondary/50 transition-colors">
                                View Profile
                              </button>
                            </div>
                          )}
                      ...
                      <div>
                        <h4 className="text-sm font-medium text-foreground/50 mb-4 flex items-center gap-2">
                      ...
                      <>
                        ...
                        {selectedJob.applicants.length > 0 && (
                          <div className="space-y-3 mb-8">
                            <h5 className="text-xs font-medium text-foreground/50 mb-3 uppercase tracking-widest">Applicants</h5>
                                    ...
                                    <div className="flex items-center gap-3 min-w-0">
                                      ...
                                      <div className="min-w-0">
                                        <div className="flex flex-wrap items-center gap-2">
                                          <p className="font-medium truncate group-hover:text-primary transition-colors">{applicant.name}</p>
                                          <StatusBadge domain="applicant" status={applicant.status || "pending_admin"} />
                                        </div>
                                      ...
                                      <div className="flex flex-wrap gap-2 justify-end">
                                        {isPending && (
                                          <>
                                            <button
                                              ...
                                              className="px-4 py-2 bg-emerald-500 text-white text-xs font-medium rounded-xl hover:bg-emerald-600 transition-colors disabled:opacity-50"
                                            >
                                              Confirm
                                            </button>
                                            {rejectDraftApplicantId === applicant.id ? (
                                              <div className="w-full sm:w-auto flex flex-col gap-2 min-w-[12rem]">
                                                ...
                                                <div className="flex gap-2 flex-wrap justify-end">
                                                  <button
                                                    ...
                                                    className="px-3 py-2 border border-secondary text-xs font-medium rounded-lg hover:bg-secondary/30"
                                                  >
                                                    Cancel
                                                  </button>
                                                  <button
                                              ...
                                              <button
                                                ...
                                                className="px-4 py-2 border border-red-500/40 text-red-600 bg-red-500/5 text-xs font-medium rounded-xl hover:bg-red-500/10 transition-colors disabled:opacity-50"
                                              >
                                                Reject
                                              </button>
                                            )}
                                        ...
                                        {(st === "rejected" || st === "withdrawn") && (
                                          <button
                                            ...
                                            disabled={assignmentBusy}
                                            onClick={() => handleAssignWorker({ id: applicant.id, name: applicant.name })}
                                            className="px-4 py-2 bg-primary text-primary-foreground text-xs font-medium rounded-xl hover:bg-primary/90 transition-colors disabled:opacity-50"
                                          >
                                            Direct assign
                                          </button>
                                        )}
                        ...
                        {canManageAssignments && suggestedWorkers.length > 0 && (
                          <div className="space-y-3 mb-8">
                            <h5 className="text-xs font-medium text-foreground/50 uppercase tracking-widest flex items-center gap-2">
                              <UserCheck className="w-4 h-4" /> Suggested matches
                            </h5>
                            <p className="text-[11px] text-foreground/50">Ranked by role fit, clearance, and reliability (manual review aid).</p>
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    <div className="flex items-center gap-2 shrink-0">
                                      <span className="text-[10px] font-medium text-primary/80 tabular-nums">+{sc}</span>
                                      <button
                                        type="button"
                                        disabled={assignmentBusy}
                                        onClick={() => handleAssignWorker({ id: w.id, name: w.name })}
                                        className="text-xs font-medium text-primary px-3 py-1.5 bg-primary/10 rounded-lg hover:bg-primary hover:text-white transition-all disabled:opacity-50"
                                      >
                                        Assign
                                      </button>
                                    </div>
                        ...
                        {canManageAssignments && (
                          <div className="pt-6 border-t border-secondary">
                            <h5 className="text-xs font-medium text-foreground/50 mb-4 uppercase tracking-widest">Manual Worker Search</h5>
                            <div className="space-y-4">
              ...
              {selectedJob.status === "Open" && !isEditing && (
                <div className="p-6 border-t border-secondary bg-background">
                  <button onClick={handleCancelJob} className="w-full py-3 border border-red-500/50 text-red-500 font-medium rounded-xl hover:bg-red-500/10 transition-colors">
                    Cancel Job Post
                  </button>
                </div>
              )}
