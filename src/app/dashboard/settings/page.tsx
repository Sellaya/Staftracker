      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <AnimatePresence>
            {saveSuccess && (
              <motion.div 
                ...
                exit={{ opacity: 0, x: -20 }}
                className="flex items-center gap-2 text-emerald-500 font-medium text-sm bg-emerald-500/10 px-3 py-1.5 rounded-full"
              >
          ...
          <button 
            onClick={handleSave}
            disabled={isSaving}
            className="flex items-center gap-2 px-6 py-2.5 bg-primary text-primary-foreground font-medium rounded-xl hover:bg-primary/90 transition-all shadow-lg shadow-primary/20 disabled:opacity-50"
          >
            {isSaving ? (
        ...
        <div className="flex-1 glass border border-secondary rounded-3xl p-6 md:p-8 bg-background/50">
          <AnimatePresence mode="wait">
            {activeTab === "general" && (
              ...
              >
                <div>
                  <h3 className="text-xl font-medium mb-4">Platform Details</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2 md:col-span-2">
                      <label className="text-sm font-medium text-foreground/70">Timezone</label>
                ...
                <div>
                  ...
                  <div className="space-y-6">
                    <div className="p-6 bg-secondary/10 border border-secondary rounded-2xl flex items-center justify-between">
                      ...
                      <button className="px-4 py-2 bg-primary text-primary-foreground rounded-lg font-medium text-sm">Enable 2FA</button>
                    </div>
                    <div className="p-6 bg-secondary/10 border border-secondary rounded-2xl flex items-center justify-between">
                      <div className="space-y-1">
                      ...
                      <button className="px-4 py-2 border border-secondary rounded-lg font-medium text-sm hover:bg-secondary/50">Revoke All</button>
                    </div>
                    <div className="space-y-4">
                      <h4 className="font-medium text-sm text-foreground/70 uppercase tracking-wider">Change Password</h4>
                      <div className="grid grid-cols-1 gap-4">
                        ...
                        <button className="bg-primary text-primary-foreground font-medium py-3 rounded-xl">Update Password</button>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            {activeTab === "notifications" && (
              ...
              >
                <div>
                  <h3 className="text-xl font-medium mb-6">Communication Preferences</h3>
                  <div className="space-y-4">
                    {[
            ...
            {activeTab === "billing" && (
              ...
              >
                <div>
                  <h3 className="text-xl font-medium mb-6">Billing & Subscription</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                    <div className="p-6 bg-primary text-primary-foreground rounded-2xl shadow-xl shadow-primary/20 relative overflow-hidden">
                      <div className="relative z-10">
                        <p className="text-sm font-medium opacity-80 mb-2">CURRENT PLAN</p>
                        <h4 className="text-3xl font-medium mb-4">Enterprise Pro</h4>
                    ...
                    <div className="p-6 border-2 border-dashed border-secondary rounded-2xl flex flex-col items-center justify-center text-center space-y-4">
                      ...
                      <button className="px-6 py-2 border border-primary text-primary rounded-lg font-medium hover:bg-primary/5 transition-colors">Talk to Sales</button>
                    </div>
                  </div>

                  <h4 className="font-medium text-lg mb-4">Client Billing Records</h4>
                      ...
                      <tbody>
                        ...
                        {invoices.map((inv) => (
                          <tr key={inv.id} className="border-b border-secondary/50 last:border-0 hover:bg-secondary/5 transition-colors">
                            ...
                            <td className="p-4">
                              <span className={`px-2 py-0.5 rounded-md font-medium text-[10px] ${inv.status === 'Paid' ? 'bg-emerald-500/20 text-emerald-500' : 'bg-amber-500/20 text-amber-500'}`}>
                                {inv.status.toUpperCase()}
                              </span>
                            </td>
                            <td className="p-4 text-right">
                              <button className="text-primary hover:underline font-medium text-xs">View Statement</button>
                            </td>
                          </tr>
                        ))}
                      </tbody>
            ...
            {activeTab === "team" && (
              ...
              >
                <div>
                  <h3 className="text-xl font-medium mb-4">Team & Roles</h3>
                      ...
                      <tbody>
                        {teamUsers.map(user => (
                          <tr key={user.id} className="border-b border-secondary/50 last:border-0">
                            ...
                            <td className="p-4">
                              <span className={`px-2 py-1 rounded-md text-xs font-medium ${user.role === 'super_admin' ? 'bg-primary/20 text-primary' : 'bg-emerald-500/20 text-emerald-500'}`}>
                  ...
                  <form onSubmit={handleCreateUser} className="p-6 bg-secondary/10 border border-secondary rounded-xl space-y-4">
                    <h4 className="font-medium mb-4 flex items-center gap-2"><Plus className="w-4 h-4"/> Add New User</h4>
                    ...
                    <button 
                      ...
                      className="mt-4 bg-primary text-primary-foreground font-medium px-6 py-2.5 rounded-lg hover:bg-primary/90 disabled:opacity-50"
                    >
                      ...
                      <div key={log.id} className="p-4 rounded-xl border border-secondary bg-secondary/5 flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div className="space-y-1">
                          <div className="flex items-center gap-2">
                            <span className="font-medium text-sm text-primary uppercase tracking-wider">{log.action}</span>
            ...
            {activeTab === "integrations" && (
              ...
              >
                ...
                <h3 className="text-2xl font-medium">Integrations Coming Soon</h3>
