  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 relative">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-medium tracking-tight">Client Management</h1>
          <p className="text-foreground/70 mt-1">Manage hospitality venues, custom rates, and billing profiles.</p>
        </div>
        <button 
          onClick={() => setIsAddModalOpen(true)}
          className="bg-primary text-primary-foreground px-4 py-2 rounded-xl text-sm font-medium shadow-lg shadow-primary/20 hover:shadow-primary/40 hover:-translate-y-0.5 transition-all"
        >
          + Add New Client
        </button>
      </div>
                ...
                >
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center text-primary font-medium">{client.name.charAt(0)}</div>
                      <div>
                ...
                {activeTab === 'overview' && (
                  <div className="grid grid-cols-2 gap-6">
                    <div className="p-4 rounded-xl border border-secondary bg-background">
                      <h4 className="text-xs font-medium text-foreground/50 mb-2">CONTACT</h4>
                    ...
                    <div className="p-4 rounded-xl border border-secondary bg-background">
                      <h4 className="text-xs font-medium text-foreground/50 mb-2">ACCOUNT</h4>
                      <button onClick={() => toggleSuspend(selectedClient.id)} className={`w-full py-2 rounded-lg text-sm font-medium border transition-colors ${selectedClient.status === "Active" ? "border-red-500 text-red-500 hover:bg-red-500/10" : "border-emerald-500 text-emerald-500 hover:bg-emerald-500/10"}`}>
                ...
                {activeTab === 'billing' && (
                  <div className="space-y-6">
                    <div className="flex justify-between items-center">
                      <h4 className="font-medium text-lg">Invoice Generation</h4>
                      <button 
                        onClick={handleGenerateInvoice}
                        disabled={isGenerating}
                        className="px-4 py-2 bg-primary text-primary-foreground font-medium rounded-xl text-sm disabled:opacity-50"
                      >
                    ...
                    <div className="space-y-3">
                      <h5 className="text-xs font-medium text-foreground/50 uppercase">History</h5>
                      {invoices.length === 0 ? (
                        ...
                        invoices.map(inv => (
                          <div key={inv.id} className="p-4 rounded-xl border border-secondary bg-background flex items-center justify-between">
                            ...
                            <div className="text-right">
                              ...
                              <p className={`text-[10px] font-medium uppercase ${inv.status === 'Paid' ? 'text-emerald-500' : 'text-amber-500'}`}>{inv.status}</p>
                            </div>
                          </div>
                        ))
                      )}
                ...
                {activeTab === 'preferred' && (
                  <div className="space-y-6">
                    <div className="flex flex-col gap-4">
                      <h4 className="font-medium text-lg">Priority Roster</h4>
                    ...
                    <div className="grid grid-cols-1 gap-3">
                      ...
                      {selectedClient.preferredWorkers.map(w => (
                        <div key={w.id} className="p-4 rounded-xl border border-secondary bg-background flex items-center justify-between group">
                          <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center text-primary text-xs font-medium">{w.name.charAt(0)}</div>
                            <span className="font-medium text-sm">{w.name}</span>
                          </div>
        ...
        {isEditModalOpen && editClientData && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-background/80 backdrop-blur-sm">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-card w-full max-w-lg rounded-2xl border border-secondary p-6 space-y-4">
              <h2 className="text-xl font-medium">Edit Client Details</h2>
              ...
              <div className="flex justify-end gap-3 pt-4">
                <button onClick={() => setIsEditModalOpen(false)} className="px-4 py-2 font-medium text-foreground/50">Cancel</button>
                <button onClick={() => handleUpdateClient(editClientData)} className="px-6 py-2 bg-primary text-primary-foreground font-medium rounded-xl">Save Changes</button>
              </div>
            </motion.div>
          </div>
        )}
        ...
        {isAddModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/80 backdrop-blur-sm">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-card w-full max-w-lg rounded-2xl border border-secondary p-6 space-y-4">
              ...
              <div className="flex justify-end gap-3 pt-4">
                <button onClick={() => setIsAddModalOpen(false)} className="px-4 py-2 font-medium text-foreground/50">Cancel</button>
                <button onClick={handleAddClient} disabled={isAdding} className="px-6 py-2 bg-primary text-primary-foreground font-medium rounded-xl disabled:opacity-50">{isAdding ? "Saving..." : "Create Client"}</button>
              </div>
            </motion.div>
          </div>
        )}
