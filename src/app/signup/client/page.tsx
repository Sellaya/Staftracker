    ...
    <div className="min-h-screen bg-background flex flex-col font-sans">
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-4 py-12">
        <div className="w-full max-w-2xl">
            ...
            {step < 5 && (
              ...
              >
                <div className="flex justify-between items-end mb-4">
                  <h1 className="text-3xl font-medium">Venue Registration</h1>
                  <span className="text-sm font-medium text-foreground/50">Step {step} of 4</span>
                </div>
          ...
          <AnimatePresence mode="wait">
              ...
              <motion.div key="step1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                <div className="p-8 glass bg-background/50 border border-secondary rounded-3xl shadow-xl hover:border-accent/30 transition-colors">
                  <div className="flex items-center gap-3 mb-6">
                    ...
                    <h2 className="text-xl font-medium">Corporate & Manager Details</h2>
                  </div>
                  <div className="space-y-4">
                    <div>
                      <label className="block text-xs font-medium text-foreground/70 mb-1">LEGAL COMPANY NAME (FOR INVOICING)</label>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-xs font-medium text-foreground/70 mb-1">WORK EMAIL</label>
                      ...
                      <div>
                        <label className="block text-xs font-medium text-foreground/70 mb-1">DIRECT PHONE NUMBER</label>
                    ...
                    <div>
                      <label className="block text-xs font-medium text-foreground/70 mb-1">A/P OR BILLING EMAIL (OPTIONAL)</label>
                ...
                <div className="flex justify-end">
                  <button onClick={nextStep} className="px-8 py-4 bg-accent text-background rounded-xl font-medium flex items-center gap-2 hover:shadow-lg hover:shadow-accent/20 transition-all">
              ...
              <motion.div key="step2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                <div className="p-8 glass bg-background/50 border border-secondary rounded-3xl shadow-xl hover:border-accent/30 transition-colors">
                  ...
                  <div className="space-y-6">
                    <div>
                      <label className="block text-xs font-medium text-foreground/70 mb-1">OPERATING VENUE NAME (PUBLIC)</label>
                    ...
                    <div>
                      <label className="block text-xs font-medium text-foreground/70 mb-2">VENUE CLASSIFICATION</label>
                    ...
                    <div>
                      <label className="block text-xs font-medium text-foreground/70 mb-1">FULL PHYSICAL ADDRESS (TORONTO & GTA)</label>
              ...
              <motion.div key="step3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                <div className="p-8 glass bg-background/50 border border-secondary rounded-3xl shadow-xl hover:border-accent/30 transition-colors">
                  <div className="flex items-center gap-3 mb-6">
                    ...
                    <h2 className="text-xl font-medium">Staffing Profile</h2>
                  </div>
                  <div className="space-y-6">
                    <div>
                      <label className="block text-xs font-medium text-foreground/70 mb-2">WHICH ROLES DO YOU TYPICALLY REQUIRE?</label>
                    ...
                    <div>
                      <label className="block text-xs font-medium text-foreground/70 mb-1">EXPECTED FREQUENCY OF NEEDS</label>
              ...
              <motion.div key="step4" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                <div className="p-8 glass bg-background/50 border border-secondary rounded-3xl shadow-xl hover:border-accent/30 transition-colors">
                  <div className="flex items-center gap-3 mb-6">
                    ...
                    <h2 className="text-xl font-medium">Venue Logistics</h2>
                  </div>
                  <div className="space-y-6">
                    
                    <div>
                      <label className="block text-xs font-medium text-foreground/70 mb-1">MANDATORY UNIFORM / DRESS CODE</label>
                    ...
                    <div>
                      <label className="block text-xs font-medium text-foreground/70 mb-1">ARRIVAL INSTRUCTIONS FOR WORKERS</label>
                ...
                <div className="flex justify-between">
                  ...
                  <button onClick={handleSubmit} disabled={isSubmitting} className="px-8 py-4 bg-accent text-background rounded-xl font-medium flex items-center gap-2 hover:shadow-lg hover:shadow-accent/20 transition-all disabled:opacity-50">
            ...
            {step === 5 && (
              <motion.div key="step5" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, x: 0 }} className="flex flex-col items-center text-center py-12">
                ...
                <h1 className="text-4xl font-medium mb-4">Registration Submitted!</h1>
                ...
                <Link href="/login/client" className="px-8 py-4 bg-accent text-background rounded-xl font-medium hover:opacity-90 transition-all">
          ...
          {stepError && (
            <div className="mt-4 p-4 border border-amber-500/30 bg-amber-500/10 text-amber-600 rounded-xl text-sm font-medium">
              {stepError}
            </div>
          )}
          {submitError && (
            <div className="mt-4 p-4 border border-red-500/30 bg-red-500/10 text-red-600 rounded-xl text-sm font-medium">
              {submitError}
            </div>
          )}
