  ...
  return (
    <div className="min-h-screen bg-background flex items-center justify-center p-4 font-sans relative overflow-hidden">
      ...
      >
        ...
        <div className="glass border border-secondary p-8 rounded-3xl shadow-2xl bg-background/40 backdrop-blur-xl">
          <form onSubmit={handleLogin} className="space-y-6">
            ...
            <button 
              type="submit" 
              disabled={isLoading}
              className="w-full bg-primary text-primary-foreground py-4 rounded-2xl font-black text-lg shadow-xl shadow-primary/20 hover:shadow-primary/40 hover:-translate-y-1 transition-all flex items-center justify-center gap-3 disabled:opacity-50 disabled:translate-y-0"
            >
              {isLoading ? (
        ...
        <p className="mt-8 text-center text-sm font-bold text-foreground/40">
          PROTECTED BY <span className="text-foreground/80">FERRARI SECURITY</span>
        </p>

        <div className="mt-6 flex justify-center">
          <Link href="/" className="text-xs font-bold text-foreground/50 hover:text-foreground transition-colors">
            Back to Public Site
          </Link>
        </div>
      </motion.div>
